  ...
  return (
    <main className="relative min-h-screen bg-stark-dark text-white selection:bg-stark-cyan selection:text-black">
      ...
      <section id="missions" className="py-24 px-8 max-w-6xl mx-auto">
            ...
            <motion.div key={index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} viewport={{ once: true }} className="group relative p-8 bg-white/5 border border-white/10 rounded-sm hover:border-stark-cyan/50 hover:bg-white/[0.08] transition-all">
              ...
              <div className="flex flex-wrap gap-2 mb-8">
                {project.tags.map(tag => (
                  <span key={tag} className="text-[9px] px-2 py-1 bg-stark-blue/10 border border-stark-blue/20 rounded text-stark-cyan uppercase font-mono">{tag}</span>
                ))}
              </div>
        ...
        {/* GITHUB CTA */}
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} className="mt-20 text-center p-12 border border-dashed border-white/10 rounded-lg">
          <h4 className="text-xl font-bold mb-4 uppercase tracking-tighter">Access Global Database</h4>
          <p className="text-gray-400 mb-8 max-w-md mx-auto text-xs italic font-mono uppercase">Decentralized Archive // 20+ Repositories Loaded</p>
          <a href="https://github.com/Arpita-2755" target="_blank" className="inline-flex items-center gap-2 px-6 py-3 bg-white text-black font-black uppercase tracking-tighter hover:bg-stark-cyan transition-colors rounded-sm text-sm">
            <Github className="w-5 h-5" /> Open GitHub Mainframe
          </a>
        </motion.div>
      </section>

      <footer className="py-12 text-center text-gray-600 font-mono text-[10px] uppercase tracking-[0.4em] border-t border-white/5">
        Arpita Mishra // Jarvis Protocol v4.2 // Silvassa, India
      </footer>
    </main>
  );
}