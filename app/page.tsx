...
import { 
  ...
  Twitter, Target, User, Send, Phone, Sun, Moon, Layers, Menu, X, Loader2, Scan
} from "lucide-react";

// --- THE DATA MAINFRAME ---
const PROFILE = {
  name: "Arpita Mishra",
  summary: "Machine Learning Engineer specializing in retrieval systems, representation learning, and applied AI system design. Specialist in building end-to-end ML pipelines spanning embedding generation, vector search optimization, and production deployment.",
...
const SKILLS = [
  ...
  { title: "Neural & GenAI", icon: <Brain className="text-stark-cyan" />, skills: ["RAG", "LLMs", "TensorFlow", "Keras", "SentenceTransformers", "HuggingFace", "Prompt Robustness"] },
  { title: "Targeting (CV)", icon: <Eye className="text-stark-gold" />, skills: ["Face Recognition", "Vector Search", "FAISS", "UMAP", "Similarity Metrics", "Threshold Calibration"] },
  { title: "Cloud & Support", icon: <Server className="text-green-400" />, skills: ["AWS", "Docker", "Git/GitHub", "Flask", "Django", "MySQL", "System Design"] }
];
...
const PROJECTS = [
  ...
  { title: "Slot Recommender", subtitle: "Healthcare NLP", description: "Converted patient queries into slots using synthetic datasets and TF-IDF extraction with Logistic Regression. 80%+ intent accuracy.", tags: ["Scikit-learn", "NLP"], github: "https://github.com/Arpita-2755", demo: null, medium: "https://medium.com/@arpitamishra2755", icon: <Activity /> },
  { title: "PromptGuard", subtitle: "LLM Robustness", description: "Deterministic prompt-mutation engine for detecting behavioral drift and response instability without labeled ground truth.", tags: ["HuggingFace", "Python"], github: "https://github.com/Arpita-2755/PromptGuard", demo: "https://promptguard-llm-robustness.streamlit.app", medium: "https://medium.com/@arpitamishra2755", icon: <Shield /> },
  { title: "IntelliCampus", subtitle: "AI Attendance", description: "Sub-second face identification using RetinaFace and FaceNet. Features auto-index rebuild logic for FAISS.", tags: ["RetinaFace", "Flask"], github: "https://github.com/Arpita-2755/IntelliCampus", demo: null, medium: "https://medium.com/@arpitamishra2755", icon: <Eye /> },
  { title: "RouteMate", subtitle: "Bus Reservation", description: "Full-stack platform using deque for seat allocation to prevent race conditions. Managed via structured Git branching.", tags: ["Flask", "MySQL"], github: "https://github.com/Arpita-2755", demo: null, medium: "https://medium.com/@arpitamishra2755", icon: <Bus /> },
  { title: "Speech Analyzer", subtitle: "Unsupervised ML", description: "NLP clustering pipeline discovering latent speech patterns via MiniLM embeddings and UMAP reduction (384 to 2).", tags: ["UMAP", "KMeans"], github: "https://github.com/Arpita-2755", demo: null, medium: "https://medium.com/@arpitamishra2755", icon: <MessageSquare /> },
  { title: "Face Verify", subtitle: "Siamese Network", description: "DL verification pipeline with 100% recall on evaluation pairs using custom Euclidean similarity layers.", tags: ["TensorFlow", "FaceNet"], github: "https://github.com/Arpita-2755", demo: null, medium: "https://medium.com/@arpitamishra2755", icon: <Cpu /> }
];
...
// --- EHA'S BRAIN ---
const EHA_KB = [
  { keywords: ["cgpa", "marks", "10th", "12th"], response: `Arpita has an 8.11 CGPA (Dean's List). School marks: 93.8% (10th) and 84.8% (12th).` },
  { keywords: ["slot", "healthcare", "synthetic"], response: "The Slot Recommender uses synthetic healthcare queries and TF-IDF with Logistic Regression for 80%+ accuracy." },
  { keywords: ["routemate", "deque", "race"], response: "RouteMate prevents race conditions in bus bookings using a queue-based (deque) allocation system." },
  { keywords: ["who", "arpita"], response: "Arpita Mishra is an ML Engineer and Visionary Architect specializing in reliability-focused AI." }
];
...
const TypewriterSummary = () => {
  ...
  const fullText = "Initialize neural links... Arpita Mishra Protocol active... ML Engineer specializing in retrieval systems and RAG architectures.";
  useEffect(() => {
    let i = 0;
  ...
  return (
    <div className="font-mono text-xs md:text-sm text-stark-cyan opacity-80 border-l-2 border-stark-cyan/30 pl-4 py-2 bg-stark-cyan/5 max-w-xl mx-auto">
...
const ArcReactor = ({ isDark }: { isDark: boolean }) => (
  <motion.div animate={{ rotate: 360 }} transition={{ duration: 15, repeat: Infinity, ease: "linear" }} className="relative w-40 h-40 flex items-center justify-center mb-6 mx-auto">
...
export default function Home() {
  ...
  const [chatHistory, setChatHistory] = useState([{ role: "eha", text: "Eha Online. Neural links active." }]);
  ...
  const handleEhaChat = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      setChatHistory(prev => [...prev, { role: "eha", text: match ? match.response : "Data point not found. Check the GitHub mainframe." }]);
    }, 450);
  };
    ...
    const interval = setInterval(() => {
      setScanProgress(prev => {
        if (prev >= 100) {
          ...
          setTimeout(() => {
            setIsScanning(false);
            window.open('/Arpita_Mishra_CV.pdf', '_blank');
          }, 500);
          return 100;
        }
        return prev + 5;
  ...
  return (
    <main className={`relative min-h-screen transition-all duration-500 ${isDark ? 'bg-[#020617] text-white' : 'bg-gray-50 text-gray-900'} selection:bg-stark-cyan overflow-x-hidden font-sans`}>
      ...
      {/* 1. SIDEBAR TOGGLE */}
      <button onClick={() => setIsSidebarOpen(true)} className="fixed top-8 left-8 z-[200] p-4 bg-white/5 backdrop-blur-lg border border-white/10 rounded-xl hover:scale-110 transition-all shadow-2xl">
        <Menu className={isDark ? 'text-stark-cyan' : 'text-stark-red'} />
      </button>

      <AnimatePresence>
        {isSidebarOpen && (
          <motion.div initial={{ x: -300 }} animate={{ x: 0 }} exit={{ x: -300 }} className={`fixed top-0 left-0 h-full w-72 z-[210] border-r backdrop-blur-3xl p-10 flex flex-col shadow-2xl ${isDark ? 'bg-black/90 border-white/10' : 'bg-white/90 border-gray-200'}`}>
            <button onClick={() => setIsSidebarOpen(false)} className="self-end mb-10 opacity-50 hover:opacity-100"><X /></button>
            ...
            <div className="mt-auto pt-10 border-t border-white/10">
              <p className="text-[10px] font-mono opacity-40 uppercase tracking-widest leading-relaxed text-center">Protocol: Mark-85<br/>Uplink: Synchronized</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {isScanning && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[300] bg-black/90 flex flex-col items-center justify-center p-10">
            <Scan className="text-stark-cyan w-20 h-20 animate-pulse mb-8" />
            <h4 className="text-2xl font-black uppercase tracking-widest text-stark-cyan mb-2">Biometric Verification...</h4>
            <div className="w-64 h-2 bg-white/10 rounded-full overflow-hidden">
               <motion.div className="h-full bg-stark-cyan" initial={{ width: 0 }} animate={{ width: `${scanProgress}%` }} />
            </div>
            <p className="mt-4 font-mono text-xs text-stark-cyan/60 italic">Deciphering identity packet for A.Mishra_CV.pdf</p>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="fixed bottom-8 right-8 z-[110]">
          ...
          {isEhaOpen && (
            <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} className={`mb-4 w-80 h-[480px] border backdrop-blur-3xl rounded-xl flex flex-col shadow-2xl ${isDark ? 'bg-black/95 border-stark-cyan/40' : 'bg-white/95 border-stark-red/40'}`}>
              <div className={`p-4 border-b flex justify-between items-center ${isDark ? 'bg-stark-cyan/10 border-white/10' : 'bg-stark-red/10 border-gray-200'}`}>
                <span className={`text-[10px] font-mono tracking-widest uppercase flex items-center gap-2 ${isDark ? 'text-stark-cyan' : 'text-stark-red'}`}><Terminal size={14} /> Eha_Link_v4</span>
                <button onClick={() => setIsEhaOpen(false)} className="opacity-40">×</button>
              </div>
              <div ref={scrollRef} className="flex-1 overflow-y-auto p-5 space-y-4 scrollbar-hide text-[11px] font-mono leading-relaxed">
                {chatHistory.map((msg, i) => (
                  <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-[85%] p-3 rounded-lg ${msg.role === "user" ? 'bg-stark-red/10 border border-stark-red/30' : (isDark ? 'bg-stark-cyan/10 border border-stark-cyan/30 text-stark-cyan' : 'bg-gray-100 text-gray-700')}`}>{msg.text}</div>
                  </div>
                ))}
              </div>
        ...
        <button onClick={() => setIsEhaOpen(!isEhaOpen)} className={`w-16 h-16 rounded-full flex items-center justify-center shadow-2xl ${isDark ? 'bg-stark-cyan shadow-stark-cyan/40' : 'bg-stark-red shadow-stark-red/40'}`}>
      ...
      <section className="relative h-screen flex flex-col items-center justify-center p-6 text-center">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
          <h2 className={`font-mono text-xs tracking-[1em] mb-4 uppercase opacity-60 ${isDark ? 'text-stark-cyan' : 'text-stark-red'}`}>Mark-85 // Arpita Mishra</h2>
      ...
      <section id="about" className="py-24 px-6 max-w-6xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-16 items-center">
          ...
          <div className="space-y-8 text-xl font-light leading-relaxed">
            <h4 className="text-5xl font-black uppercase tracking-tighter">The Visionary Architect</h4>
            <p className="italic font-bold text-2xl">"Efficiency is the only currency. If a system is noisy, it is failed armor."</p>
            <p>{PROFILE.summary}</p>
      ...
      <section id="missions" className="py-24 px-6 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {PROJECTS.map((p, i) => (
            <div key={i} className={`group relative border p-10 flex flex-col h-full transition-all rounded-xl ${isDark ? 'bg-stark-dark border-white/10 hover:border-stark-cyan/40 shadow-2xl' : 'bg-white border-gray-200 hover:shadow-2xl'}`}>
        ...
        <div className="mt-16 text-center p-12 border-2 border-dashed border-white/10 rounded-2xl group hover:border-stark-cyan/40 transition-all">
          ...
          <a href="https://github.com/Arpita-2755" target="_blank" className={`px-10 py-5 font-black uppercase rounded-lg transition-all ${isDark ? 'bg-white text-black hover:bg-stark-cyan' : 'bg-stark-red text-white hover:bg-red-500 shadow-xl'}`}>Open GitHub Database</a>
        </div>
      </section>
